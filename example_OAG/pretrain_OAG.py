...
# train_range = {t: True for t in graph.times if t != None and t > 2016 and t <= 2017}
...
data, rem_edge_list, ori_edge_list, _, _ = GPT_sample(randint(), pre_target_nodes, pre_range, args.batch_size,
                                                      feature_OAG)
    ...
    for batch in np.arange(repeat_num) + 1:
        ...
        with torch.no_grad():
            ...
            print((
                  ...
                  (epoch, batch, repeat_num, (st - et), optimizer.param_groups[0]['lr'], np.average(train_link_losses),
                   np.average(train_attr_losses), \
